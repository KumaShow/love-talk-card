...
**本專案範例**：
- `card-data.schema.json` — 卡牌資料檔結構（初始版本為 `cards.json`；現行架構為 `src/data/themes/*.json` 聚合）
- `game-session.schema.json` — sessionStorage 序列化格式
- `pwa-manifest.json` — PWA Web App Manifest 規格

**重要**：contracts 不是文件，是 **可執行的驗證規則**。tasks.md 會產生「用 ajv 或 Zod 驗證卡牌資料符合 schema」這種測試任務。
...
| 所有文字外部化 | i18n / data 檔 | PASS | 主題資料 JSON + i18n/*.json |
```
...
contracts 需包含卡牌資料 schema、game-session schema、PWA manifest。
```
...
- Phase 2: Foundational（types、路由、store 骨架、卡牌資料）
