...
> 最後更新：YYYY-MM-DD
> 對應專案範圍：`src/types/index.ts`、`src/data/themes/*.json`、`src/utils/theme.ts`
...
- literal union 把「業務上只允許這五種」的規則下放到型別層級。
...
```ts
export const validThemeIds: ThemeId[] = ['attraction', 'self', 'interaction', 'trust', 'desire']
```
...
```ts
export const validThemeIds = ['attraction', 'self', 'interaction', 'trust', 'desire'] as const
// 型別變成 readonly ['attraction', 'self', 'interaction', 'trust', 'desire']
```
...
```ts
const validThemeIds = ['attraction', 'self', 'interaction', 'trust', 'desire'] as const
type ThemeId = typeof validThemeIds[number]  // 'attraction' | 'self' | 'interaction' | 'trust' | 'desire'
```

好處：**新增主題只要改陣列一個地方**，型別自動跟上。本專案目前以 `types/index.ts` 的 `VALID_THEME_IDS` 作為來源，`utils/theme.ts` 再沿用這份清單。
