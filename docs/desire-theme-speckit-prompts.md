...
若你尚未在 `clarify` 前先手動定義細節，建議先以以下預設前進，減少後續反覆改 spec：

1. `desire` 主題先沿用既有每主題 **20 張** 配比，也就是 `15 base + 5 intimate`。
2. 原有四主題的 `intimate` **不升級為更露骨內容**，只允許微調提示文案，維持「溫和私密」定位。
3. `desire` 主題在首頁 **直接顯示**，但主題預覽與說明文案需清楚標示其為成人親密主題。
4. 主題命名在 spec 階段先固定使用 `desire`，避免 plan 與 tasks 階段仍用暫名導致檔名、型別與 i18n key 漂移。
5. Success Criteria 以 **產品邊界清楚 + 使用者預期明確 + 技術結構可維護** 三者並重，不只看內容文案本身。
6. 雙語、可外部化、可翻譯要求應在 spec 階段就明寫，不要等到 implement 才補。

### 如果你已經先跑過 `/speckit.specify`

若 `spec.md` 已經生成，這份文件仍然可以繼續用，但角色會從「開局提示詞」變成「後續收斂與補強清單」：

1. 先人工檢查現有 `spec.md` 是否已明確寫出：
   - 為何新增第 5 主題，而不是直接提高原 `intimate` 尺度
   - `desire` 與四主題的邊界
   - 可接受內容與禁止內容
   - 成人內容提示策略
   - 與原四主題 `intimate` 的相容策略
2. 若上述任一點缺漏，優先在 `/speckit.clarify` 或手動修正 spec 時補齊，不要把模糊點帶進 `/speckit.plan`。
3. 之後可直接從本文件的 `/speckit.clarify` 段落往下走，不必重跑 `/speckit.specify`。
...
如果你已先跑過 `/speckit.specify`，這一步尤其重要，因為它會直接決定：

1. 後續資料模型是否仍保留固定配比
2. 首頁與預覽要不要加入成人內容提示
3. `desire` 是否在本輪就視為正式主題命名
4. tests 與 schema 要改到什麼程度
...
7. desire 主題是否需要在主題預覽或進入前顯示成人內容提示，而不是只在首頁名稱暗示
...
8. desire 主題若需成人提示，應在資料層、i18n、UI 哪一層持有這個訊號
9. 是否需要讓主題卡或預覽支援額外 metadata，例如成人提示標記或注意事項文案
...
6. 包含首頁或主題預覽的成人內容提示任務（若 clarify / spec 已定案）
7. 不要把本次需求拆成過多非必要 Phase
...
6. 先人工檢查 tasks 是否合理
7. `/speckit.implement`
8. 接近完成時再視情況補 `/speckit.checklist`

### `spec.md` 人工檢查重點

若你已先產出 spec，建議至少確認以下幾點都存在：

1. `desire` 的定位不是單純「更辣的 intimate」
2. 與四主題的邊界有明確區分
3. 有寫出禁止內容與同意邊界
4. 有寫出成人提示或預期管理方式
5. 有寫出與原四主題 `intimate` 的相容策略
6. 有把雙語、可外部化納入需求或成功條件
