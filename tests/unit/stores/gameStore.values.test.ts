...
describe('values game session', () => {
  ...
  it.each([false, true])('intimateMode=%s 時仍建立相同 25 張 values deckOrder', (intimateMode) => {
    const store = useGameStore()
    store.startSession('values', intimateMode)

    expect(store.deck).toHaveLength(25)
    expect(store.deck.map((card) => card.id).sort()).toEqual(
      Array.from({ length: 25 }, (_, index) => `val-${String(index + 1).padStart(3, '0')}`),
    )
