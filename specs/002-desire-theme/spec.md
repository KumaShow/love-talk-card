...
**Independent Test**: 可透過首頁與主題預覽獨立測試：使用者不需要進入遊戲，就能從主題名稱、簡述與提示文字辨識 desire 是成人親密主題，並理解其內容尺度高於既有 intimate 模式。
...
4. **Given** 使用者不想進行成人身體親密對話，**When** 使用者看到 desire 的預覽提示，**Then** 使用者應能在進入前做出不選擇此主題的決定。
...
**Independent Test**: 可透過主題說明與卡牌內容審查獨立測試：既有四主題與其 intimate 卡仍維持原定位；直接身體親密與性需求內容只出現在 desire 主題。
...
- 如果使用者已啟用 intimate 模式後再選擇 desire，系統應如何避免使用者誤解為「雙重加辣」？使用者預期應維持清楚：desire 本身即是成人親密主題，intimate 不應讓其越過安全與同意邊界。
- 如果某張卡牌同時包含情感信任與身體親密，系統應如何歸類？應以題目的主要對話任務判斷；若重點是身體親密中的同意、界線或需求協商，歸入 desire。
- 如果使用者只想要浪漫但不想談性，系統應如何協助避開 desire？首頁與預覽應在進入前提供足夠清楚的成人親密提示。
...
- **FR-003**: System MUST distinguish `desire` from intimate mode: intimate mode remains a gentler private layer within each theme, while `desire` is a standalone adult intimacy theme with its own user expectation and content boundary.
- **FR-004**: System MUST communicate before entry that `desire` contains adult intimacy topics, using clear, respectful, non-alarmist language on the homepage and/or theme preview.
...
- **FR-008**: System MUST keep all user-facing `desire` labels, descriptions, warnings, card text, and closing messages externalizable and translatable, with Traditional Chinese and English supported at minimum.
...
- **Desire Theme**: A fifth theme dedicated to adult physical intimacy and consent-centered desire conversations. It is opt-in, clearly labeled, and bounded by safety rules.
- **Card**: A conversation prompt belonging to exactly one theme. A desire card uses the same maintainable card structure as other themes while following stricter adult-content boundary rules.
- **Intimate Mode**: A mode that adds gentler private cards to themes. It remains compatible with the new theme system but is not redefined as a direct adult sexuality mode.
- **Adult Intimacy Notice**: User-facing copy shown before entering desire to set expectations about adult content, consent, and optional participation.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: First-time users can identify before entering gameplay that `desire` is an adult intimacy theme, not a general romance or emotional-trust theme.
...
- **SC-004**: 100% of `desire` user-facing copy is externalizable and available in at least Traditional Chinese and English before release.
...
- **SC-006**: The theme model remains consistent enough that adding `desire` does not require a separate content pipeline from the existing themes.
- **SC-007**: Users can opt into or avoid `desire` from the homepage/theme preview without changing their ability to use existing themes and intimate mode.
...
- Existing four themes and intimate mode are valuable and should be preserved rather than retroactively rewritten into a higher-intensity adult mode.
- Traditional Chinese is the source language for product tone, with English maintained as a required supported language.
- Future implementation may decide the exact number of `desire` base/intimate cards, but this specification only defines product behavior, content boundaries, and acceptance criteria.
