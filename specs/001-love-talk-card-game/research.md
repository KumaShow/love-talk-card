...
**Decision**: Tailwind CSS 4.x，utility-first，搭配 CSS custom property 管理主題色

**Rationale**:
Tailwind 的 JIT 編譯確保未使用的類別完全不進入 bundle，壓縮後 CSS 極小。主題色以 CSS 自訂變數（`--color-theme-bg`、`--color-theme-primary` 等）管理，主題切換只需在根元素更新 CSS 變數值，搭配 `transition` 實現 300–500ms 平滑過渡，無需 JavaScript 重新計算樣式。觸控目標 `min-w-[44px] min-h-[44px]` 可直接以 Tailwind utility class 強制執行，WCAG 合規性易於驗證。
...
- 字體：`Playfair Display`（WOFF2）
- 音效：`flip.ogg`、`flip.mp3`、`bgm.ogg`、`bgm.mp3`
...
VitePWA({
  ...
  workbox: {
    globPatterns: ['**/*.{js,css,html,json,woff2,png,svg,ogg,mp3}'],
  },
  manifest: {
    display: 'standalone',
