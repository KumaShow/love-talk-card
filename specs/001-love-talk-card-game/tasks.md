...
- **2026-04-23（PR #8 Copilot review 回應 + T088 收尾）**：PR #8（branch `chore/phase8-quality-acceptance`）Copilot Code Review 對 `perf.spec.ts` 提 3 點建議，以 commit `3890607` 回應：(a) `runCommand` 補 `child.on('error')` 並改用 `close` 取代 `exit`，對齊 `us5-offline-pwa.spec.ts` 寫法避免 spawn 失敗時 Promise hang；(b) 新增 `test.describe.configure({ timeout: 180_000 })` 使 `beforeAll` 跑 `npm run build` + 等 preview 不受全域 30s 限制；(c) 去除 `page.waitForTimeout(500)` 硬等，改 `evaluate(transitionend Promise)` + `expect.not.toHaveClass(/picked-shell-enter-active/)` 事件式等待。PR 合併後 GitHub Actions workflow run #11（head `70f9baa`）**completed + success**，線上 `https://kumashow.github.io/love-talk-card/` 回 HTTP 200，`<html lang="zh-TW">` + 註冊 `registerSW.js` + 掛 `<link rel="manifest">`；另外 `registerSW.js`、`sw.js`、`manifest.webmanifest`、`assets/index-*.js` 皆回 200（bundle hash 與本地 build 一致）。至此 Phase 8（T077–T088）全部收尾。
...
- [x] T088 推送至 `main` 分支並驗證 .github/workflows/deploy.yml 的 GitHub Actions workflow 完整通過所有工作：lint → type-check → test → build → deploy；確認線上網址可載入帶有 Service Worker 的遊戲
