...
- [x] T061 [US4] 在 src/assets/main.css（由 main.ts 匯入）新增全域 CSS 過場：`body, #app { transition: background-color 500ms ease-in-out, background 500ms ease-in-out; }`，並在 `#app` 上設定 `background: linear-gradient(to bottom, var(--color-bg), var(--color-bg-end));`
